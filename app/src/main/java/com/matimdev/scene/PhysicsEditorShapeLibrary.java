        ...
        for(final FixtureTemplate fixtureTemplate : bodyTemplate.fixtureTemplates) {
            ...
            for(int i = 0; i < fixtureTemplate.polygons.length; i++) {
                ...
                final FixtureDef fixtureDef = fixtureTemplate.fixtureDef;
                 ...
                 Vector2[] samba = fixtureTemplate.polygons[i].vertices;
                
               
                	scaleX = (pShape.getScaleX()*pShape.getWidth())/(pShape.getWidth());
                	scaleY = (pShape.getScaleY()*pShape.getHeight())/(pShape.getHeight());
               
              
//                if(scaleX > 1.0f)
//                {
//                	 
//                    for(int j = 0;j<samba.length;j++)
//                    {
//                    	samba[j].x  = samba[j].x *  scaleX;
//                    }
//                }
//                else
//                {
//                	 for(int j = 0;j<samba.length;j++)
//                     {
//                     	samba[j].x  = (samba[j].x / 1.0f) * scaleX  ;
//                     }
//                }
//                
//                
...
//                	 for(int j = 0;j<samba.length;j++)
//                     {
//                     	samba[j].y  = (samba[j].y/1.0f) *  scaleY;
//                     }
//                }
